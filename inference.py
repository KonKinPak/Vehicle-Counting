...
def visualize_image(filename, image,
  ...
  count_vehicle(filename,classes,scores,category_index)
  img = np.array(image)
...
def count_vehicle(filename,classes,scores,category_index):
  n = 0
  # print("filename = ",filename)
  for i in range(len(classes)):
    if scores[i] > 0.3:
      if category_index[classes[i]]['name'] in ['bicycle','car','motorcycle','airplane','bus','train','truck','boat']:
        # print(category_index[classes[i]]['name'])
        n+=1
  f = open(f"./counting/efficientdet-d6.txt","a+")
...
def visualize_image_prediction(filename, image,
                               prediction,
                               label_map=None,
                               **kwargs):
  ...
  return visualize_image(filename,image, boxes, classes, scores, label_map, **kwargs)
...
class ServingDriver(object):
  ...
  def visualize(self,filename, image, prediction, **kwargs):
    """Visualize prediction on image."""
    return visualize_image_prediction(filename,
        image,
        prediction,
