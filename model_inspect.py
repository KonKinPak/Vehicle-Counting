...
flags.DEFINE_integer('max_boxes_to_draw', 200, 'Max number of boxes to draw.')
flags.DEFINE_float('min_score_thresh', 0.25, 'Score threshold to show box.')
...
class ModelInspector(object):
  ...
  def saved_model_inference(self, image_path_pattern, output_dir, **kwargs):
    ...
    for i in range(num_batches):
      ...
      # print(all_files)
      file_name = all_files[i][13:-4]
      for j in range(size_before_pad):
        img = driver.visualize(file_name,raw_images[j], detections_bs[j], **kwargs)
